...
/**
 * ModelCostos — Módulo incorporado desde dentavitalis (B)
 *
 * Persistencia en bd de la tabla tba_centrocostos, tba_costo, y tba_detallecosto.
 */
class ModelCostos
{
  ...
  {
    $statement = Conexion::conn()->prepare("SELECT tba_detallecosto.IdDetalleCosto, tba_detallecosto.IdGasto, tba_detallecosto.NumeroDocumento, tba_detallecosto.ObservacionGasto, tba_detallecosto.FechaCosto, tba_detallecosto.PrecioGasto, tba_gasto.NombreGasto FROM $tabla INNER JOIN tba_gasto ON tba_detallecosto.IdGasto = tba_gasto.IdGasto WHERE tba_detallecosto.IdCosto = $codCosto");
    $statement->execute();
    return $statement->fetchAll();
  }
  ...
  {
    // Convertir formato de fecha: si viene "2026-05", convertir a "05-2026"
    $mesFormato = date('m-Y', strtotime($mesActual . '-01'));
    $statement = Conexion::conn()->prepare("SELECT COALESCE(SUM(TotalCosto), 0) AS suma_mes FROM $tabla WHERE MesCosto LIKE ? OR MesCosto = ?");
    $statement->execute(array($mesFormato, $mesActual));
    return $statement->fetch();
  }
  ...
  public static function mdlSumarMayorCentroCostos($tabla)
  {
    $statement = Conexion::conn()->prepare("
      SELECT 
        ...
        SUM(c.TotalCosto) AS SumaMayorCosto 
      FROM tba_centrocostos cc 
      LEFT JOIN tba_costo c ON cc.IdCentroCostos = c.IdCentroCostos 
      GROUP BY cc.IdCentroCostos, cc.DescripcionCentro 
      ORDER BY SUM(c.TotalCosto) DESC 
      LIMIT 1
    ");
    $statement->execute();
    return $statement->fetch();
  ...
  public static function mldMostrarSumaCostosPorMeses($tabla, $fechaInicial, $fechaFinal)
  {
    $statement = Conexion::conn()->prepare(" 
      SELECT 
        cc.IdCentroCostos, 
        cc.DescripcionCentro, 
        COALESCE(SUM(dc.PrecioGasto), 0) AS SumaTotalCosto
      FROM tba_centrocostos cc 
      INNER JOIN tba_costo c ON cc.IdCentroCostos = c.IdCentroCostos 
      INNER JOIN tba_detallecosto dc ON c.IdCosto = dc.IdCosto 
      WHERE dc.FechaCosto >= ? AND dc.FechaCosto <= ? 
      GROUP BY cc.IdCentroCostos, cc.DescripcionCentro
      ORDER BY SumaTotalCosto DESC
    ");
    $statement->execute(array($fechaInicial, $fechaFinal));
    return $statement->fetchAll();
  }
  ...
  public static function mldMostrarSumaCostosPorCentro($tabla, $codCentroCosto)
  {
    $statement = Conexion::conn()->prepare("
      ...
      FROM tba_costo c 
      WHERE c.IdCentroCostos = ? 
      GROUP BY c.MesCosto 
      ORDER BY c.MesCosto DESC
    ");
    $statement->execute(array($codCentroCosto));
    return $statement->fetchAll();
  }
