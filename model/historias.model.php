...
/**
 * ModelHistorias — Fusionado ortho-center (A) + dentavitalis (B)
 *
 * Decisión: se usa el esquema de B como base (historia narrativa + detalle clínico separado).
 * La tabla tba_historiaclinica usa IdSocio (en lugar de IdUsuario) como FK al médico.
 *
 * Cambios respecto a ortho-center original:
 *  - mdlMostrarAllHistorias: usa IdSocio e incluye TipoIdentificacion/NumeroIdentificacion
 *  - mdlCrearHistoriaClinica: esquema B (narrativo: AlergiasEncontradas, DatosInformante,
 *    TiempoEnfermedad, SignosSintomas, RelatoCronologico, etc.) — sin checkboxes de A
 *  - [NUEVO B] mdlCrearDetalleHistoria: inserta en tba_detallehistoriaclinica (signos vitales)
 *  - mdlMostrarCabeceraHistoria: devuelve campos narrativos de B
 *  - [NUEVO B] mdlMostrarDetalleHistoria: devuelve signos vitales y diagnóstico
 *  - mdlUpdateHistoriaClinica: actualiza campos narrativos de B
 *  - [NUEVO B] mdlUpdateDetalleHistoria: actualiza tba_detallehistoriaclinica
 *  - mdlBuscarHistoriaDNI: usa NumeroIdentificacion (antes DNIPaciente)
 *  - [NUEVO B] mdlVerificarUsoSocio: verifica si un socio tiene historias
 *  - [NUEVO B] mdlVerificarHistoriaPaciente: cuenta historias de un paciente
 *  - [NUEVO B] mdlCrearHistoriaCotizacion: inserta historia mínima desde cotización
 *  - [NUEVO B] mdlCrearDetalleHistoriaCotizacion: inserta detalle mínimo desde cotización
 */
class ModelHistorias
{
  // ── LISTAR ──────────────────────────────────────────────────────────────

  // Mostrar todas las historias clínicas con datos del paciente y médico
  public static function mdlMostrarAllHistorias($tabla)
  {
    $statement = Conexion::conn()->prepare("SELECT tba_historiaclinica.IdHistoriaClinica, tba_historiaclinica.IdPaciente, tba_historiaclinica.IdSocio, tba_historiaclinica.FechaActualiza, tba_paciente.NombrePaciente, tba_paciente.ApellidoPaciente, tba_paciente.TipoIdentificacion, tba_paciente.NumeroIdentificacion, COALESCE(tba_usuario.NombreUsuario, 'Sin asignar') as NombreUsuario FROM $tabla INNER JOIN tba_paciente ON tba_historiaclinica.IdPaciente = tba_paciente.IdPaciente LEFT JOIN tba_usuario ON tba_historiaclinica.IdSocio = tba_usuario.IdUsuario ORDER BY IdHistoriaClinica DESC");
    $statement->execute();
    return $statement->fetchAll();
  }

  // Mostrar cabecera de la historia (anamnesis narrativa)
  ...
  // [NUEVO B] Mostrar detalle clínico de la historia (signos vitales, diagnóstico)
  ...
  // Obtener el ID de la última historia clínica creada
  ...
  // Obtener el código de historia por código de paciente
  ...
  {
    $statement = Conexion::conn()->prepare("SELECT COUNT(*) AS Contador FROM $tabla INNER JOIN tba_paciente ON tba_historiaclinica.IdPaciente = tba_paciente.IdPaciente WHERE tba_paciente.NumeroIdentificacion = $numeroDNI");
    $statement->execute();
    return $statement->fetch();
  }

  // Obtener ruta del odontograma
  ...
  // Crear cabecera de historia clínica (modelo narrativo de B)
  public static function mdlCrearHistoriaClinica($tabla, $datosCreateHistoria)
  {
    $statement = Conexion::conn()->prepare("INSERT INTO $tabla (IdPaciente, IdSocio, IdUsuario, AlergiasEncontradas, DatosInformante, MotivoConsulta, TiempoEnfermedad, SignosSintomas, RelatoCronologico, FuncionesBiologicas, AntecedentesFamiliares, AntecedentesPersonales, UsuarioCreado, UsuarioActualizado, FechaCreado, FechaActualiza) VALUES(:IdPaciente, :IdSocio, :IdUsuario, :AlergiasEncontradas, :DatosInformante, :MotivoConsulta, :TiempoEnfermedad, :SignosSintomas, :RelatoCronologico, :FuncionesBiologicas, :AntecedentesFamiliares, :AntecedentesPersonales, :UsuarioCreado, :UsuarioActualizado, :FechaCreado, :FechaActualiza)");
    ...
    $statement->bindParam(":IdUsuario",              $datosCreateHistoria["UsuarioCreado"],          PDO::PARAM_STR);
  ...
  // [NUEVO B] Crear detalle clínico de la historia (signos vitales, diagnóstico)
  ...
  // [NUEVO B] Crear historia mínima desde cotización (sin datos clínicos aún)
  public static function mdlCrearHistoriaCotizacion($tabla, $datosCreate)
  {
    $statement = Conexion::conn()->prepare("INSERT INTO $tabla (IdPaciente, IdSocio, IdUsuario, UsuarioCreado, UsuarioActualizado, FechaCreado, FechaActualiza) VALUES(:IdPaciente, :IdSocio, :IdUsuario, :UsuarioCreado, :UsuarioActualizado, :FechaCreado, :FechaActualiza)");
    $statement->bindParam(":IdPaciente",         $datosCreate["IdPaciente"],         PDO::PARAM_STR);
    $statement->bindParam(":IdSocio",            $datosCreate["IdSocio"],            PDO::PARAM_STR);
    $statement->bindParam(":IdUsuario",          $datosCreate["UsuarioCreado"],      PDO::PARAM_STR);
    $statement->bindParam(":UsuarioCreado",      $datosCreate["UsuarioCreado"],      PDO::PARAM_STR);
    $statement->bindParam(":UsuarioActualizado", $datosCreate["UsuarioActualizado"], PDO::PARAM_STR);
    $statement->bindParam(":FechaCreado",        $datosCreate["FechaCreado"],        PDO::PARAM_STR);
    $statement->bindParam(":FechaActualiza",     $datosCreate["FechaActualiza"],     PDO::PARAM_STR);
    return $statement->execute() ? "ok" : "error";
  }

  // [NUEVO B] Crear detalle mínimo de historia desde cotización
  public static function mdlCrearDetalleHistoriaCotizacion($tabla, $datosCreate)
  {
    $statement = Conexion::conn()->prepare("INSERT INTO $tabla (IdHistoriaClinica, IdTratamiento, FechaCreado, FechaActualiza, UsuarioCreado, UsuarioActualizado) VALUES(:IdHistoriaClinica, :IdTratamiento, :FechaCreado, :FechaActualiza, :UsuarioCreado, :UsuarioActualizado)");
    $statement->bindParam(":IdHistoriaClinica",  $datosCreate["IdHistoriaClinica"],  PDO::PARAM_STR);
    $statement->bindParam(":IdTratamiento",      $datosCreate["IdTratamiento"],      PDO::PARAM_STR);
    $statement->bindParam(":FechaCreado",        $datosCreate["FechaCreado"],        PDO::PARAM_STR);
    $statement->bindParam(":FechaActualiza",     $datosCreate["FechaActualiza"],     PDO::PARAM_STR);
    $statement->bindParam(":UsuarioCreado",      $datosCreate["UsuarioCreado"],      PDO::PARAM_STR);
    $statement->bindParam(":UsuarioActualizado", $datosCreate["UsuarioActualizado"], PDO::PARAM_STR);
  ...
  // Actualizar cabecera de historia clínica (campos narrativos)
  ...
  // [NUEVO B] Actualizar detalle clínico de la historia
  ...
  // Actualizar ruta del odontograma
  ...
  // Eliminar historia o detalle por IdHistoriaClinica (tabla como parámetro)
  ...
  // Verificar uso de un paciente en historias clínicas
  ...
  // [NUEVO B] Verificar uso de un socio/médico en historias clínicas
  ...
  // [NUEVO B] Verificar si existe historia clínica para un paciente
  public static function mdlVerificarHistoriaPaciente($tabla, $codPaciente)
  {
    $statement = Conexion::conn()->prepare("SELECT COUNT(IdHistoriaClinica) AS TotalHistorias FROM $tabla WHERE tba_historiaclinica.IdPaciente = $codPaciente");
    $statement->execute();
    return $statement->fetch();
  }
}