...
/**
 * ModelVisitas — Fusionado ortho-center (A) + dentavitalis (B)
 *
 * Mezclando la columna IdPago y TotalPagado (de A)
 * junto con ObservacionVisita (de B) en todas las consultas y registros.
 */
class ModelVisitas
{
  // Mostrar lista de visitas por paciente (A+B unidos)
  ...
  FROM $tabla
    LEFT JOIN tba_detalletratamiento
      ON tba_visita.IdDetalleTratamiento = tba_detalletratamiento.IdDetalleTratamiento
    LEFT JOIN tba_procedimiento
      ON tba_detalletratamiento.IdProcedimiento = tba_procedimiento.IdProcedimiento
    LEFT JOIN tba_pago
      ON tba_visita.IdPago = tba_pago.IdPago
  WHERE tba_visita.IdHistoriaClinica = $codHistoria");
    $statement->execute();
    return $statement->fetchAll();
  }

  // Ingresar nuevas visitas (A+B: inserta IdPago y ObservacionVisita)
  public static function mdlIngresarVisitas($tabla, $datosCreate)
  {
    $statement = Conexion::conn()->prepare("INSERT INTO $tabla (IdHistoriaClinica, IdPago, IdDetalleTratamiento, MotivoVisita, ObservacionVisita, FechaVisita, UsuarioCreado, UsuarioActualiza, FechaCreacion, FechaActualizacion) VALUES(:IdHistoriaClinica, :IdPago, :IdDetalleTratamiento, :MotivoVisita, :ObservacionVisita, :FechaVisita, :UsuarioCreado, :UsuarioActualiza, :FechaCreacion, :FechaActualizacion)");
    ...
    $statement->bindParam(":ObservacionVisita",    $datosCreate["ObservacionVisita"],    PDO::PARAM_STR);
  ...
  // Obtener el código de pago vinculado (Lógica A)
  ...
  // Obtener el historial de visitas completo (Lógica A mejorada con ObservacionVisita)
  ...
  // Eliminar todas las visitas de un solo codigo de historia clinica (Lógica A)
