...
/**
 * ControllerGastos — Módulo incorporado de dentavitalis (B)
 *
 * Módulo secundario utilizado por la tabla de pagos/costos para 
 * catalogar tipos de gastos fijos.
 */
class ControllerGastos
{
  ...
  public static function ctrCrearGasto()
  {
    if (isset($_POST["nombreGasto"])) {
      ...
      if ($respuesta == "ok") {
        echo '<script>Swal.fire({ icon: "success", title: "Correcto", text: "Gasto ingresado Correctamente!" }).then(function(r){ if(r.value) window.location = "gastos"; });</script>';
      } else {
        echo '<script>Swal.fire({ icon: "error", title: "Error", text: "Error al ingresar gasto" }).then(function(r){ if(r.value) window.location = "gastos"; });</script>';
      }
    }
  }
  ...
  public static function ctrEditarGasto()
  {
    if (isset($_POST["editarNombreGasto"])) {
      ...
      if ($respuesta == "ok") {
        echo '<script>Swal.fire({ icon: "success", title: "Correcto", text: "Gasto editado Correctamente!" }).then(function(r){ if(r.value) window.location = "gastos"; });</script>';
      } else {
        echo '<script>Swal.fire({ icon: "error", title: "Error", text: "Error al editar gasto" }).then(function(r){ if(r.value) window.location = "gastos"; });</script>';
      }
    }
  }
  ...
  public static function ctrEliminarGasto()
  {
    if (isset($_GET["codGasto"])) {
      ...
      if ($respuesta == "ok") {
        echo '<script>Swal.fire({ icon: "success", title: "Correcto", text: "Gasto eliminado Correctamente!" }).then(function(r){ if(r.value) window.location = "gastos"; });</script>';
      }
    }
  }
