...
/**
 * ControllerSocios — Nuevo módulo incorporado de dentavitalis (B)
 *
 * Maneja los médicos y demás personal (tiposocio).
 */
class ControllerSocios
{
  ...
  public static function ctrCrearSocio()
  {
    if (isset($_POST["nombreSocio"])) {
      ...
      if ($respuesta == "ok") {
        echo '<script>Swal.fire({ icon: "success", title: "Correcto", text: "Socio ingresado Correctamente!" }).then(function(r){ if(r.value) window.location = "socios"; });</script>';
      } else {
        echo '<script>Swal.fire({ icon: "error", title: "Error", text: "Error al ingresar el socio" }).then(function(r){ if(r.value) window.location = "socios"; });</script>';
      }
    }
  }
  ...
  public static function ctrEditarSocio()
  {
    if (isset($_POST["editarNombreSocio"])) {
      ...
      if ($respuesta == "ok") {
        echo '<script>Swal.fire({ icon: "success", title: "Correcto", text: "Socio editado Correctamente!" }).then(function(r){ if(r.value) window.location = "socios"; });</script>';
      } else {
        echo '<script>Swal.fire({ icon: "error", title: "Error", text: "Error al editar el socio" }).then(function(r){ if(r.value) window.location = "socios"; });</script>';
      }
    }
  }
  ...
  public static function ctrEliminarSocio()
  {
    if (isset($_GET["codSocio"])) {
      ...
      if (($confirmarUsoCosto["TotalUso"] > 0) || ($confirmarUsoHistoria["TotalUso"] > 0)) {
        echo '<script>Swal.fire({ icon: "error", title: "Error", text: "¡No se puede eliminar el socio, está en uso en historias clínicas o en costos!" }).then(function(r){ if(r.value) window.location = "socios"; });</script>';
      } else {
        $tabla = "tba_socio";
        $respuesta = ModelSocios::mdlEliminarSocio($tabla, $codSocio);
        if ($respuesta == "ok") {
          echo '<script>Swal.fire({ icon: "success", title: "Correcto", text: "¡Socio eliminado Correctamente!" }).then(function(r){ if(r.value) window.location = "socios"; });</script>';
        } else {
          echo '<script>Swal.fire({ icon: "error", title: "Error", text: "No se pudo eliminar el socio" }).then(function(r){ if(r.value) window.location = "socios"; });</script>';
        }
      }
    }
  }

  // Mostrar usuarios socio para listado de gastos (reportes)
  ...
  // Mostrar socios por el tipo de socio que es (ej: 1 = Médicos)
  ...
  public static function ctrCrearTipoSocio()
  {
    if (isset($_POST["nombreTipoSocio"])) {
      if ($_POST["nombreTipoSocio"] != '') {
        ...
        if ($respuesta == "ok") {
          echo '<script>Swal.fire({ icon: "success", title: "Correcto", text: "Se creó correctamente el tipo de socio" }).then(function(r){ if(r.value) window.location = "socios"; });</script>';
        } else {
          echo '<script>Swal.fire({ icon: "error", title: "Error", text: "No se pudo crear el tipo de socio" }).then(function(r){ if(r.value) window.location = "socios"; });</script>';
        }
      } else {
        echo '<script>Swal.fire({ icon: "error", title: "Error", text: "Tipo de socio no válido" }).then(function(r){ if(r.value) window.location = "socios"; });</script>';
      }
    }
  }
}
